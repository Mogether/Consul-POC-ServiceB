package com.poc.serviceb.web.rest;
...
import com.poc.serviceb.IntegrationTest;
import com.poc.serviceb.config.EmbeddedKafka;
import com.poc.serviceb.config.KafkaSseConsumer;
import com.poc.serviceb.config.KafkaSseProducer;
...
class ServiceBKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/service-b-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/service-b-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/service-b-kafka/unregister"));
                return;
            }
        }
